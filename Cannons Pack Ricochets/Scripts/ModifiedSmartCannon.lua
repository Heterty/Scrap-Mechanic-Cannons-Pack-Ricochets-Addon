...
dofile("$CONTENT_c0344d93-7492-46c8-88be-a61699e57041/Scripts/Cannons_Pack_libs/ScriptLoader.lua")
dofile("GlobalScripts/CPProjCustom.lua")

_cpEffect_addEffectInfo(
	"967bdaf1-3c00-4f52-b43c-02379d2e095c",
	{
		rld = "Reloading",
...
_cpEffect_addEffectInfo(
	"e6c0634f-47ff-4b21-817b-747bd679f682",
	{
		rld = "Reloading",
		sht_snd = "NumberLogicCannon - Shoot",
		sht = "SmartCannon - MuzzleFlash1"
...
_cpCannons_addCannonInfo(
	"967bdaf1-3c00-4f52-b43c-02379d2e095c",
	{
		ejected_shell_id = _cpProj_getShellEjectorEnum("MediumShell"),
		proj_data_id = _cpProj_getProjEnum("SmartCannon"),
...
ModifiedSmartCannon = class(GLOBAL_SCRIPT)
ModifiedSmartCannon.maxParentCount = -1
ModifiedSmartCannon.maxChildCount  = 1
ModifiedSmartCannon.connectionInput  = _connectionType.logic + _connectionType.power
ModifiedSmartCannon.connectionOutput = _connectionType.logic
ModifiedSmartCannon.colorNormal    = _colorNew(0x5d0096ff)
ModifiedSmartCannon.colorHighlight = _colorNew(0x9000e8ff)

function ModifiedSmartCannon:client_onCreate()
	self:client_injectScript("CPProjCustom")
...
function ModifiedSmartCannon:getDefaultProjectileId()
...
function ModifiedSmartCannon:server_onCreate()
	self.sv_settings = {
		number = {
			...
			[NumLogicTrTable.ricochet_chance      ] = 1.0,
			[NumLogicTrTable.ricochet_angle       ] = 70
		},
		logic = {
...
function ModifiedSmartCannon:client_getAvailableParentConnectionCount(connectionType)
...
function ModifiedSmartCannon:client_getAvailableChildConnectionCount(connectionType)
...
function ModifiedSmartCannon:server_onFixedUpdate()
	...
	if cannon_active and not self.reload then
		...
		if not spudgun_mode then
			...
			self.projConfig[336] = math.random(0, sm.game.getCurrentTick()) --Ricochet seed
			self.projConfig[337] = _NumSettings[NumLogicTrTable.ricochet_chance] --Ricochet chance
			self.projConfig[338] = _NumSettings[NumLogicTrTable.ricochet_angle] --Ricochet angle
			...
			for i = 1, projectile_per_shot + 1, 1 do
				...
				CPProjCustom:server_sendProjectile(self, self.projConfig, self.proj_data_id)
			end
		else
...
function ModifiedSmartCannon:client_receiveCannonData(data)
	self:client_GUI_LoadNewData(data)
end

function ModifiedSmartCannon:server_requestNumberInputs(data, player)
	self.data_request_queue[#self.data_request_queue + 1] = player
end

function ModifiedSmartCannon:server_requestCannonData(data, player)
...
function ModifiedSmartCannon:server_prepareEffectTable()
	local sv_set = self.sv_settings
	return {
...
function ModifiedSmartCannon:server_requestSound(data, caller)
...
function ModifiedSmartCannon:server_setNewSettings(data)
	local sv_set = self.sv_settings
...
function ModifiedSmartCannon:client_setEffects(data)
...
function ModifiedSmartCannon:client_onInteract(character, state)
	if not state then return end

	self:client_GUI_Open()
end

function ModifiedSmartCannon:client_onFixedUpdate(dt)
...
function ModifiedSmartCannon:client_canInteract()
...
function ModifiedSmartCannon:client_onDestroy()
	self:client_GUI_onDestroyCallback()
end

function ModifiedSmartCannon:client_displayEffect(data)
...
function ModifiedSmartCannon:client_GUI_Open()
...
function ModifiedSmartCannon:client_GUI_onDestroyCallback()
	local gui = self.gui
...
function ModifiedSmartCannon:client_GUI_onGetValueCallback()
...
function ModifiedSmartCannon:client_GUI_onSaveButtonCallback()
...
function ModifiedSmartCannon:client_GUI_onResetDefaultsCallback()
	local s_gui = self.gui
...
function ModifiedSmartCannon:client_GUI_onNumPageChange(btn_name)
...
function ModifiedSmartCannon:client_GUI_CreateTempValTable()
	local temp = {}
	temp[1] = --number logic
	{
		...
		[4]  = {name = "Ricochet Chance 0-1"           , type = sc_gui_num_val , id = NumLogicTrTable.ricochet_chance      , value = 0, default = 0.5 , min = 0, max = 1 },
		[5]  = {name = "Ricochet Angle(90 = no ricochet)"                , type = sc_gui_num_val , id = NumLogicTrTable.ricochet_angle       , value = 0, default = 70  , min = 0, max = 90 },
		[6]  = {name = "Explosion Level"               , type = sc_gui_num_val , id = NumLogicTrTable.expl_level           , value = 0, default = 5   , min = 0.001, max = 99999999},
		[7]  = {name = "Explosion Radius (m)"          , type = sc_gui_num_val , id = NumLogicTrTable.expl_radius          , value = 0, default = 0.5 , min = 0.1, max = 100},
		[8]  = {name = "Explosion Impulse Radius (m)"  , type = sc_gui_num_val , id = NumLogicTrTable.expl_impulse_radius  , value = 0, default = 15.0, min = 0.001, max = 99999999},
		[9]  = {name = "Explosion Impulse Strength"    , type = sc_gui_num_val , id = NumLogicTrTable.expl_impulse_strength, value = 0, default = 2000, min = 0, max = 99999999},
		[10]  = {name = "Projectile Gravity"            , type = sc_gui_num_val , id = NumLogicTrTable.projectile_gravity   , value = 0, default = 10  , min = -99999999, max = 99999999},
		[11] = {name = "Projectile Lifetime (s)"       , type = sc_gui_num_val , id = NumLogicTrTable.projectile_lifetime  , value = 0, default = 15  , min = 0.001, max = 30},
		[12] = {name = "Recoil"                        , type = sc_gui_num_val , id = NumLogicTrTable.cannon_recoil        , value = 0, default = 0   , min = 0, max = 99999999},
		[13] = {name = "Proximity Fuze (m)"            , type = sc_gui_num_val , id = NumLogicTrTable.proximity_fuze       , value = 0, default = 0   , min = 0, max = 20},
		[14] = {name = "Projectiles Per Shot"          , type = sc_gui_num_val , id = NumLogicTrTable.projectile_per_shot  , value = 0, default = 0   , min = 0, max = 20, int = true},
		[15] = {name = "X Projectile Offset (m)"       , type = sc_gui_num_val , id = NumLogicTrTable.x_offset             , value = 0, default = 0   , min = -99999999, max = 99999999},
		[16] = {name = "Y Projectile Offset (m)"       , type = sc_gui_num_val , id = NumLogicTrTable.y_offset             , value = 0, default = 0   , min = -99999999, max = 99999999},
		[17] = {name = "Z Projectile Offset (m)"       , type = sc_gui_num_val , id = NumLogicTrTable.z_offset             , value = 0, default = 0   , min = -99999999, max = 99999999},
		[18] = {name = "Projectile Type (Spudgun Mode)", type = sc_gui_list_val, id = NumLogicTrTable.projectile_type      , value = 0, default = 0   , min = 0, max = projectile_type_count, list = projectile_type_table}
	}
...
function ModifiedSmartCannon:client_GUI_TabCallback(tab_name)
...
function ModifiedSmartCannon:client_GUI_updateTabButtons()
	local s_gui = self.gui
...
function ModifiedSmartCannon:client_GUI_getCurrentOption(btn_id)
	local s_gui = self.gui
...
function ModifiedSmartCannon:client_GUI_onListBoxChangeCallback(btn_name)
...
function ModifiedSmartCannon:client_GUI_onBooleanChangedCallback(btn_name)
	local btn_id = tonumber(btn_name:sub(-1))
...
function ModifiedSmartCannon:client_GUI_onTextChangedCallback(widget, text)
...
function ModifiedSmartCannon:client_GUI_updateCurrentTab()
	local s_gui = self.gui
	local cur_temp = s_gui.temp[s_gui.cur_tab]
...
function ModifiedSmartCannon:client_GUI_updateCurrentPageText()
	local s_gui = self.gui
	local gui_int = s_gui.interface
	local cur_page = s_gui.cur_page
...
function ModifiedSmartCannon:client_GUI_LoadNewData(data)
	local gui = self.gui
	if not gui then return end
...
function ModifiedSmartCannon:client_GUI_SetWaitingState(state)
	local gui = self.gui
	local gui_int = gui.interface
...
function ModifiedSmartCannon:client_GUI_UpdateDotAnim()
	local gui = self.gui
	if not gui then return end
